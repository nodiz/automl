...
flags.DEFINE_string('coco_json', None, 'Coco json')
...
class ModelInspector(object):
        ...
        if runmode == 'dry':
            ...
            self.saved_model_benchmark(
                ...
                trace_filename=kwargs.get('trace_filename', None))
                         ...
                         'saved_model_video', 'coco_agnostic_eval'):
            config_dict = {}
            ...
            if runmode == 'saved_model':
                ...
                self.saved_model_video(kwargs['input_video'], kwargs['output_video'],
                                       **config_dict)
            elif runmode == 'coco_agnostic_eval':
                self.agnosticCoco(kwargs['input_image'], kwargs['coco_json'], **config_dict)
    ...
    def agnosticCoco(self, input_image, coco_json, **kwargs):
        ...
        for cat in imageAnnotations['categories']:
            ...
            print('Doing', id, name)
            image_file_names = [os.path.join(input_image, img['file_name'])
                                for img in imageAnnotations['images']
                                if img['categories'] == [id]] # for test taking only three images
            ...
            for i in range(num_batches):
                ...
                res = dict(file=fnames[0], height=height, width=width, detections=[])
                # creating compatible boxes:
                for j in range(boxes.shape[0]):
                    if scores[j] > kwargs['min_score_thresh']:
                        res['detections'].append(dict(
                            conf=scores[j],
            ...
            if results:
                ...
                coco_metrics = coco_eval(results, coco_json.replace('animals', 'animal'), save=True, boxTransform='oid')
