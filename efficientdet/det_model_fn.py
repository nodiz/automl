...
def asymmetric_loss(y_pred, y_true, gamma_neg=4, gamma_pos=1, clip=0.05, eps=1e-8):
    """Compute the focal loss between `logits` and the golden `target` values.
    ...
    Returns:
      loss: A float32 scalar representing normalized total loss.
    """
    with tf.name_scope('asymmetric_loss'):
        # compute focal loss multipliers before label smoothing, such that it will
        # not blow up the loss.
        ...
        x_pos = pred_prob
        x_neg = 1 - pred_prob
        x_pos_shift = tf.clip_by_value(x_pos, eps, 1)
        x_neg_shift = tf.clip_by_value(x_neg + clip, eps, 1)

        # Basic CE calculation
        log_pos = y_true * tf.math.log(x_pos_shift)
        log_neg = (1 - y_true) * tf.math.log(x_neg_shift)
        loss = log_pos + log_neg

        # Asymmetric Focusing
        p_t = (y_true * x_pos_shift) + (1 - y_true) * x_neg_shift
        one_gamma = gamma_pos * y_true + gamma_neg * (1 - y_true)
        one_w = (1.0 - p_t) ** one_gamma
...
def detection_loss(cls_outputs, box_outputs, labels, params):
    ...
    for level in levels:
        ...
        if "asl_loss" in params and params['asl_loss']:
            cls_loss = asymmetric_loss(cls_outputs[level], cls_targets_at_level)
        else:
            cls_loss = focal_loss(
                cls_outputs[level],
